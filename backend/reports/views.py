"""
MonthlyReport ViewSet - Core CRUD operations for Monthly Reports.

This ViewSet handles core CRUD operations (list, destroy).
Additional functionality is split into focused ViewSets:
- views_matrix.py: Matrix data and member details
- views_download.py: Excel and PALMS file downloads
...
class MonthlyReportViewSet(viewsets.GenericViewSet):
    """
    Core CRUD ViewSet for MonthlyReport operations.

    Provides basic operations:
    - list: Get all monthly reports for a chapter (IsChapterOrAdmin)
    - destroy: Delete a monthly report (IsAdmin only)

    Additional functionality in separate ViewSets:
    - MonthlyReportMatrixViewSet: Member details, TYFCB data
    - MonthlyReportDownloadViewSet: Excel/PALMS downloads
    - MonthlyReportAggregationViewSet: Multi-month aggregation

    Authentication: IsChapterOrAdmin
    - Chapters can access their own reports
    - Admins can access all reports
    - Delete operation requires IsAdmin
    """
